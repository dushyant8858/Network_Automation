...
def sigint_handler(signum, frame):
	...
	for root, dirs, files in os.walk('/home/jaimeen/Downloads/proxy/cache/'):
  		for f in files:
...
fh = open( "/home/jaimeen/Downloads/proxy/blocked.txt", "r")			#open file which contains list of blocked websites
blocked2 = fh.readlines()												#read its contents into a variable

fh.close()

fh2 = open( "/home/jaimeen/Downloads/proxy/blocked.txt", "a")

for i in blocked2:
	i = i[:-1]
	print("address being sent to query " + i)
	try:
		answers = dns.resolver.query(i, 'CNAME')
		for rdata in answers:
			if str(rdata.target) in blocked:
				pass
			else:
				fh2.write(str(rdata.target) + '\n')
				print("name written in file" + str(rdata.target))
	except:
...
def proxy_server(webserver, port, conn, addr,data, url ):
	...
	if os.path.isfile("/home/jaimeen/Downloads/proxy/cache/" + hashname):		#check if cache for requested url exists
		...
		try:
			...
			#print(mtime)
			if (mtime < float(time.time() - float(timetrack[hashname]))) or (revalidation[hashname] == "must-revalidate"):		#check if cache is not fresh
	...
	if not (os.path.isfile("/home/jaimeen/Downloads/proxy/cache/" + hashname)) or flag == False:
		try:
			...
			http_response="HTTP/1.1 400 Bad Request\nConnection: Keep-Alive\nKeep-Alive: max=5, timeout=10 \n\n <html><body>400 Bad request reason: Invalid URL ></body></html>"
			conn.send(http_response.encode())			#check if url is invalid
			conn.close()			
		...
		try:
			...
			if not "no-cache" in buffr or not "no-store" in buffr or not "Cache-Control: private" in buffr:				#if "no-cache" present in reply then do not cache the file, else cache it
				savecache(url, buffr)
			s.close()
			conn.close()
			...
			#print(buffr)
			if not "no-cache" or not "no-store" in buffr:
...
def savecache(url ,buffr):							#function to save url in form of a cache file
	...
	try:
		...
		if "must-revalidate" in buffr:
			revalidation[hashname] = "must-revalidate"
		else:
			revalidation[hashname] = ""			
	...
	try:
		...
		for i in range(0, int(a1)):
			if "Link: " in buffr:
				...
				if "rel=prefetch" in t1:						#fetch link only if rel-prefetch is stated
					link = buffr.split("Link: ")[1].split(" ")[0].split("<")[1].split(">")[0]
					buffr = buffr.split("Link: ")[1]
					url = url + link						#extract link and join it to url
					http_msg = "GET " + url + " HTTP/1.0\n\n"
		...
		for i in range(0, int(a2)):
			if "link rel=\"prefetch\"" in buffr:			#fetch link only if rel-prefetch is stated
				...
				while 1:
					...
					if len(reply2) > 0:
						...
						break
				savecache(a1 ,buffr2)				#save response in buffer and send it to savecache function
				s2.close()
